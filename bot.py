...
VIP_PACKAGES = {
    ...
    'month': {'stars': 100, 'days': 30, 'label': '1 Month VIP',      'emoji': '\U0001f31f'},
    'test':  {'stars': 1,   'days': 1,  'label': 'Test VIP (Admin)', 'emoji': '\U0001f9ea'},
}
...
# FIX A: No f-string SQL in grant_vip — parameterized interval
async def grant_vip(uid, days):
    await db_pool.execute(
        '''UPDATE users SET is_vip=TRUE,
           vip_expiry=GREATEST(NOW(),COALESCE(vip_expiry,NOW()))+(($2::text)||' days')::interval
           WHERE user_id=$1''',
        uid, str(int(days)))
...
def get_tod_count(context):
    return context.user_data.get('tod_count', 0)

def increment_tod_count(context):
    context.user_data['tod_count'] = context.user_data.get('tod_count', 0) + 1

def clear_tod_count(context):
    context.user_data.pop('tod_count', None)
            ...
            if pref:
                row = await conn.fetchrow(
                    '''SELECT w.user_id FROM waiting_users w
                       ...
                       ORDER BY w.queued_at LIMIT 1 FOR UPDATE OF w SKIP LOCKED''',
                    pref, my_gender, uid)
            else:
                row = await conn.fetchrow(
                    '''SELECT w.user_id FROM waiting_users w
...
async def stop_chat(update, context):
    ...
    await update.message.reply_text(
        ...
        reply_markup=get_main_keyboard(uid))
    await update.message.reply_text('Was there a problem?',
        reply_markup=report_inline(partner))
    try:
        await context.bot.send_message(partner,
...
async def delete_blocked_command(update, context):
    ...
    for r in rows:
        ...
        try: await context.bot.send_chat_action(chat_id=uid, action='typing'); await asyncio.sleep(0.05)
        except Forbidden:
            for q in [
                'DELETE FROM waiting_users WHERE user_id=$1',
                'DELETE FROM active_chats  WHERE user_id=$1',
...
async def stats_command(update, context):
    ...
    top_refs = await db_pool.fetch('SELECT name,username,referral_count FROM users WHERE referral_count>0 ORDER BY referral_count DESC LIMIT 5')
    mp = round(male/total*100) if total else 0
    fp = round(female/total*100) if total else 0
    inchat_m  = await db_pool.fetchval("SELECT COUNT(*) FROM users u WHERE u.gender='Male' AND EXISTS(SELECT 1 FROM active_chats ac WHERE ac.user_id=u.user_id)")
    inchat_f  = await db_pool.fetchval("SELECT COUNT(*) FROM users u WHERE u.gender='Female' AND EXISTS(SELECT 1 FROM active_chats ac WHERE ac.user_id=u.user_id)")
    search_m  = await db_pool.fetchval("SELECT COUNT(*) FROM users u WHERE u.gender='Male' AND EXISTS(SELECT 1 FROM waiting_users w WHERE w.user_id=u.user_id)")
    search_f  = await db_pool.fetchval("SELECT COUNT(*) FROM users u WHERE u.gender='Female' AND EXISTS(SELECT 1 FROM waiting_users w WHERE w.user_id=u.user_id)")
    lines = [
        '📊 Full Stats\n',
        f'👤 Total users:    {total}',
        f'👨 Male:           {male} ({mp}%)',
        ...
        f'\n💬 Active chats:  {active}',
        f'🔎 Waiting:        {waiting}',
        f'👨💬 In chat males:     {inchat_m}',
        f'👩💬 In chat females:   {inchat_f}',
        f'👨🔎 Searching males:   {search_m}',
        f'👩🔎 Searching females: {search_f}',
...
async def tod_callback(update, context):
    ...
    if data.startswith('tod_start:'):
        ...
        if not is_vip and count >= FREE_TOD_LIMIT:
            await q.answer(f'Free users get {FREE_TOD_LIMIT} rounds. Get 💎 VIP for unlimited!', show_alert=True); return
        await q.edit_message_text('🎲 You challenged your partner!')
        try: await context.bot.send_message(partner, '🎲 Stranger challenged you!\n\nYou pick:', reply_markup=tod_choice_inline(uid))
        except: pass
...
async def soulmate_reveal_callback(update, context):
    ...
    await q.answer()
    partner = await get_partner(uid)
    if not partner: await q.edit_message_text('⚠️ You are not in a chat anymore.'); return
    await db_pool.execute(
        '''INSERT INTO soulmate_reveals(user_id,partner_id) VALUES($1,$2)
           ON CONFLICT(user_id) DO UPDATE SET partner_id=$2, created_at=NOW()''', uid, partner)
    await q.edit_message_text(
        '✅ You tapped Reveal!\n\nWaiting for your partner...',
        parse_mode='Markdown')
    partner_row = await db_pool.fetchrow(
        'SELECT 1 FROM soulmate_reveals WHERE user_id=$1 AND partner_id=$2', partner, uid)
    if not partner_row: return
...
async def successful_payment_handler(update, context):
    ...
    pkg_key = parts[1] if len(parts) == 2 else None
    pkg     = VIP_PACKAGES.get(pkg_key)
    if not pkg: logger.error('Unknown VIP payload: %s uid=%s', payment.invoice_payload, uid); return
...
async def testvip_command(update, context):
    ...
    await update.message.reply_text('🧪 Test invoice sent! Pay 1 Star to verify the full payment flow.')
...
async def soulmate_command(update, context):
    ...
    try: await context.bot.send_message(partner, msg, parse_mode='Markdown', reply_markup=soulmate_inline(partner))
    except: pass
...
async def router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Sync username on every message
                ...
                if text and text.isdigit() and 5 <= int(text) <= 120:
                    ...
                    try:
                        ...
                        if referrer and not already:
                            ...
                            if vip_granted:
                                try: await context.bot.send_message(referrer, f'🎉 You earned {VIP_REFERRAL_DAYS} days of 👑 VIP for inviting friends!')
                                except: pass
    ...
    if uid == ADMIN_ID and context.user_data.get('in_admin_panel'):
        ...
        if text == '📱 Live Chats':
            pairs = await db_pool.fetch('SELECT DISTINCT LEAST(user_id,partner_id) AS u1, GREATEST(user_id,partner_id) AS u2 FROM active_chats ORDER BY u1')
            ...
            for i, pair in enumerate(pairs):
                ...
                buttons.append([InlineKeyboardButton(f'🚫 End #{i+1}: {n1} & {n2}', callback_data=f'admin_end_chat:{u1}:{u2}')])
            await update.message.reply_text('\n'.join(lines), reply_markup=InlineKeyboardMarkup(buttons)); return
        if text == '👑 VIP Users':
            rows = await db_pool.fetch("SELECT user_id,username,name,vip_expiry,referral_count FROM users WHERE (is_vip=TRUE AND vip_expiry IS NULL) OR vip_expiry>NOW() ORDER BY vip_expiry ASC NULLS FIRST LIMIT 20")
        ...
        if context.user_data.get('announce_mode'):
            if text == '⬅️ Back':
                context.user_data.pop('announce_mode', None); context.user_data.pop('announce_target', None)
            ...
            if target == 'female':
                rows = await db_pool.fetch("SELECT user_id FROM users WHERE is_banned=FALSE AND name IS NOT NULL AND age IS NOT NULL AND gender='Female'")
            else:
                rows = await db_pool.fetch('SELECT user_id FROM users WHERE is_banned=FALSE AND name IS NOT NULL AND age IS NOT NULL')
            sent = 0; blocked = 0
            for r in rows:
        ...
        if text == '⬅️ Back':
            context.user_data['in_admin_panel'] = False; context.user_data.pop('announce_mode', None)
            await update.message.reply_text('Main menu', reply_markup=admin_main_keyboard); return

    # ── BUTTONS ──
    if text == '🚀 Find Partner': context.user_data['last_pref'] = None; await match_user(update, context); return
    if text == '👨 Find Male':
    ...
    if text == '⭐ Buy VIP':
        ...
        if uid == ADMIN_ID:
            buttons.append([InlineKeyboardButton(f"🧪 {VIP_PACKAGES['test']['label']} — {VIP_PACKAGES['test']['stars']} Star", callback_data='buy_vip:test')])
        await update.message.reply_text(
    ...
    # ── RELAY — FIX B: rate limiting ──
    if text not in BUTTON_TEXTS:
        # FIX B: Rate limit — max 1 message per second per user
        now = time.time()
        ...
        if partner:
            try:
                ...
                await db_pool.execute('DELETE FROM active_chats WHERE user_id=$1', uid)
                await db_pool.execute('DELETE FROM active_chats WHERE user_id=$1', partner)
                await update.message.reply_text('👋 Your partner has left.\n\nPress 🚀 Find Partner to meet someone new!',
                    reply_markup=get_main_keyboard(uid))
            ...
            await update.message.reply_text('Not in a chat. Press 🚀 Find Partner.', reply_markup=get_main_keyboard(uid))

# ── START ──
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.message.from_user.id; username = update.message.from_user.username
    if await is_banned_check(uid): await update.message.reply_text('🚫 You are banned.'); return
    ref = None
    ...
    if uid == ADMIN_ID:
        await db_pool.execute(
            "INSERT INTO users(user_id,username,name,gender,is_vip) VALUES($1,$2,'Admin','Male',TRUE) ON CONFLICT(user_id) DO UPDATE SET is_vip=TRUE,vip_expiry=NULL",
            uid, username)
        await db_pool.execute('DELETE FROM active_chats  WHERE user_id=$1', uid)
        await db_pool.execute('DELETE FROM waiting_users WHERE user_id=$1', uid)
        context.user_data.clear()
    ...
    if await user_exists(uid):
        ...
        return
    context.user_data.clear()
    context.user_data['step'] = 'name'; context.user_data['ref'] = ref; context.user_data['username'] = username
...
async def main():
    ...
    for attempt in range(1, 11):
        try:
            await app.initialize(); await app.start(); break
        except Exception as e:
